...
static inline void add_text_field_fn_(char **var, char **updates) {
  *var = *updates;
  *updates = NULL;
}

#define add_text_field(meta, updates, field)                                   \
  add_text_field_fn_(&(meta)->field, &(updates)->field)
...
int tja_metadata_update_(tja_metadata *meta, tja_metadata *updates) {
  ...
  if (isnan(meta->demostart))
    meta->demostart = updates->demostart;

  meta->side = updates->side;
  meta->course = updates->course;
  if (!isnan(updates->bpm))
    meta->bpm = updates->bpm;
  if (!isnan(updates->offset))
    meta->offset = updates->offset;
  if (!isnan(updates->level))
    meta->level = updates->level;
